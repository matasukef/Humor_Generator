...
def get_data_exp1(data_path, image_dir, num):
    ...
    images = [os.path.join(image_dir, image.strip()).split(
        '/', 1)[1] for image in data.iloc[:, 1]]
...
def get_data_exp2(data_path, image_dir, num):
    ...
    result = {'images': images, 'cap_ll': humor_ll, 'cap_lm': humor_lm, 'cap_lh': humor_lh, 'cap_ml': humor_ml,
              'cap_mm': humor_mm, 'cap_mh': humor_mh, 'cap_hl': humor_hl, 'cap_hm': humor_hm, 'cap_hh': humor_hh}
